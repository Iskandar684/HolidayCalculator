                  Продукт учета отгулов.

Система позволяет вести учет отгулов в электронном виде. 
Пользователь формирует заявление на отгул и отправляет на утверждение.
Пользователь с полномочием принятия решения утверждает или отклоняет.
Одновременно, при подачи электронного заявления формируется и печатная форма, которую можно распечатать при необходимости.
Также можно подать заявление на отпуск или на отзыв (перевод дней, в которые сотрудник отработал во время отпуска в отгула).
Администратор с логином admin добавляет пользователей и назначает им полномочия.

Имеются 2 версии "клиента": 
-настольная (на Java, на платформе Eclipse RCP)
-веб (на Javascript с использованием jquery)
Сервер: wildfly
БД: postgresql 

Экранные формы приложения описаны в проекте balzaq mockups:
/HolidayCalculator-docs/mockups/HolidayCalculator.bmpr

Для автоматической установки серверной части продукта запустите следующий скрипт в ОС Linux:
/HolidayCalculator/holiday.calculator.deploy/deploy.sh

Далее инструкция по РУЧНОМУ разворачиванию сервера. 
